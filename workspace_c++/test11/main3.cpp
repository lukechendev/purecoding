...
void replaceInOrder_old(vector<int>& v, int toRemove, int toAdd) {
    if (toRemove == toAdd) {
...
int activityNotifications(vector<int> expenditure, int d) {
    ...
    sort(trail);
    list<int> trailList(trail.begin(), trail.end());
    for (int i = d; i < size; ++i) {
        if (i > d) {
            replaceInOrder_old(trail, expenditure[i - d - 1], expenditure[i - 1]);
        }
        auto m1 = trailList.begin();
        for (int j = 0; j < (iMedian1 - 1); ++j) {
          m1++;
        }

        int median2Times = *m1;
        if (iMedian1 != iMedian2) {
          m1++;
        }
        median2Times += *m1;
