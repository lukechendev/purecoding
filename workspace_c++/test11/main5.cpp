...
size_t findV(const vector<int>& v, size_t lo, size_t hi, int value) {
    size_t mid = (lo + hi) / 2;
    if (value < v[mid]) {
      return findV(v, lo, mid - 1, value);
    } else if (value > v[mid]) {
      return findV(v, mid + 1, hi, value);
    } else {
      return mid;
    }
}

size_t findVPos(const vector<int>& v, size_t lo, size_t hi, int value) {
    if (lo == hi) {
      return lo;
    }
    ...
    size_t mid = (lo + hi) / 2;
    if (value <= v[mid]) {
      return findVPos(v, lo, mid, value);
    } else {
      return findVPos(v, mid + 1, hi, value);
    }
}
...
void replaceInOrder(vector<int>& v, int toRemove, int toAdd) {
    ...
    size_t iToRemove = findV(v, 0, v.size() - 1, toRemove);
    size_t iToAdd = findVPos(v, 0, v.size() - 1, toAdd);
