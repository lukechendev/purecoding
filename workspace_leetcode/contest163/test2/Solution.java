...
class FindElements {
  ...
  private TreeSet<Integer> valList = new TreeSet<Integer>();
  
  private void buildTreeNode(TreeNode node, int val) {
    if (node == null) {
      return;
    }
    ...
    buildTreeNode(node.left, 2 * val + 1);
    buildTreeNode(node.right, 2 * val + 2);
  }
